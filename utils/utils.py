...
def transactions_executed():
    ...
    try:
        for i in range(
                len(data)):  ### Проход по всему списку на поиск и добавление в новый список по статусу "Успешных операций"
            try:
...
def hidden_account_from(operation_sort, number):
    ...
    if split_from[0].lower() == "maestro" or split_from[0].lower() == 'mastercard':
        ...
        hidden_output = split_from[0] + ' ' + hidden_account[:4] + ' ' + hidden_account[4:8] + ' ' + hidden_account[
                                                                                                     8:12] + ' ' + hidden_account[
                                                                                                                   12:16] + ' ' + hidden_account[
                                                                                                                                  16:20]
        ...
        hidden_output = split_from[0] + ' ' + split_from[1] + ' ' + hidden_account[:4] + ' ' + hidden_account[
                                                                                               4:8] + ' ' + hidden_account[
                                                                                                            8:12] + ' ' + hidden_account[
                                                                                                                          12:16] + ' ' + hidden_account[
                                                                                                                                         16:20]
...
def hidden_account_to(operation_sort, number):
    ...
    if split_to[0].lower() == "maestro" or split_to[0].lower() == 'mastercard':
        ...
        hidden_output = split_to[0] + ' ' + hidden_account[:4] + ' ' + hidden_account[4:8] + ' ' + hidden_account[
                                                                                                   8:12] + ' ' + hidden_account[
                                                                                                                 12:16] + ' ' + hidden_account[
                                                                                                                                16:20]
        ### Вывод информации после переборки текста split_from[0] Это Maestro/MasterCard, hidden_account разделяется по 4-е символа
        ...
        hidden_output = split_to[0] + ' ' + split_to[1] + ' ' + hidden_account[:4] + ' ' + hidden_account[
                                                                                           4:8] + ' ' + hidden_account[
                                                                                                        8:12] + ' ' + hidden_account[
                                                                                                                      12:16] + ' ' + hidden_account[
                                                                                                                                     16:20]
        ...
        for i in range(len(split_to[1])):
            if i in range(0, 16):
                ...
                hidden_account += split_to[1][i]
        hidden_output = split_to[0] + ' ' + hidden_account[-6:]
        ### Вывод последних 6-ти символов, при этом первые 16 из 20 символов заменены на звёздочку
        return hidden_output


def date_output(operation_sort, number):
    ''' Вывод информации о дате операции в формате ДД.ММ.ГГГГ и вида платежа'''
    date = datetime.datetime.fromisoformat(operation_sort[number]['date']).date()
    return (f"{date.day}.{date.month}.{date.year} {operation_sort[number]['description']}")


def transfer_amount_currency(operation_sort, number):
    ''' Вывод информации о сумме и валюте операции '''
    return (
        f'{operation_sort[number]["operationAmount"]["amount"]} {operation_sort[number]["operationAmount"]["currency"]["name"]}\n')
